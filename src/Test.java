...
public class Test {     //类功能：建立模型并求解
    ...
    public Test(Data data ){ this.data = data; }
    ...
    public void solve_model() throws IloException{
        ...
        ArrayList<ArrayList<Integer>> load = new ArrayList<>();//车离开停靠点时的车上的人数序列
        ...
        solution = new Solution(data,routes,start_serve,stop_times,stop_demands,load);
...
//        }
        ...
        for(int i=1;i<=data.stop_num;i++){
            for(int k=0;k<data.vec_num;k++){
                ...
                for(int j = 0; j < 2+2*data.stop_num; j++){
                    if(data.arcs[i][j]==1 && data.arcs[i+data.stop_num][j]==1) {
        ...
        for(int k=0;k<data.vec_num;k++){
            for(int i=1;i<=2*data.stop_num;i++){
                ...
                for(int j = 0; j < 2+2*data.stop_num; j++){
                    if(data.arcs[i][j]==1 && data.arcs[j][i]==1) {
            ...
            for(int i=0;i<2+2*data.stop_num;i++){
                for(int j = 0; j < 2+2*data.stop_num; j++){
                    if(data.arcs[i][j]==1) {
                        ...
                        expr10 = model.prod(M, model.sum(1, model.prod(-1, x[i][j][k])));
            ...
            for(int i=0;i<2+2*data.stop_num;i++){
                for(int j = 0; j < 2+2*data.stop_num; j++){
                    if(data.arcs[i][j]==1 ) {
                        ...
                        expr10 = model.prod(M, model.sum(1, model.prod(-1, x[i][j][k])));
                        expr12 = model.sum(q[j], model.sum(w[i][k], model.prod(-1, w[j][k])));
    ...
    public static void main(String[] args) throws Exception {
        Data data = new Data();
        String path = "data/test1.txt";       //算例地址
        proData(path,data);
        System.out.println("Input Successfully...");
        System.out.println("cplex procedure......");
        Test cplex = new Test(data);
        cplex.build_model();
        ...
        System.out.println(" cplex_time " + cplex_time + " best_cost " + cplex.total_cost);
    }
}